...
There are two options as the starting point:

- RNA-seq raw FASTQ files (paired-end).
  
- Aligned BAM files (see below for requirements of BAM file)
...
If you are not sure whether your RNA-seq library is stranded or not, please run the short diagnostic script named submit_strandness.sh. This script runs on BAM files, so the first step is to run the alignment pipeline if the starting point is FASTQ files (see below in [Configure](#configure-and-run)). We use infer_experiment.py from [RSeQC](https://rseqc.sourceforge.net/). 
...
If you start from FASTQ files, modify the job file submit_star_alignment_template.sh in the main work directory. For this step, you need the following: 
...
```
qsub submit_splicing_junction_pipeline_[cohort name].sh   # or equivalent command for your scheduler
```


If you stat from aligned BAM files or you have generated BAM files as above, modify the job file submit_splicing_junction_pipeline_template.sh in the main work directory. You need the following:  

...
```
tar -xzvf eQTLGen_splicing_junction_v2.tar.gz
```
...
|   |-- eQTLGen_splicing_junction_v2.tar.gz
|   |-- eQTLGen_splicing_junction_v2.tar.gz.md5
|   |-- output
|   |-- singularity_img
|   |   |-- eqtlgen_splicing_junction_strandness.sif
|   |   |-- eqtlgen_splicing_junction_v2.sif
|   |   `-- eqtlgen_splicing_star_v2.sif
|   `-- splicing_junction_extension
|       |-- README.md
|       |-- conf
...
There are two options as the starting point and a helper script to infer strand-specificity of RNA-seq library (see details in [Strand-specificity](#strand-specificity))):
...
By default, we assume the genome assembly used for read alignment is hg38. Please re-generate BAM files using this workflow if you are using a different version (e.g. hg19). 

BAM files generated by STAR (recommended) can be used. Importantly, if your library is unstranded (see [Strand-specificity](#strand-specificity))) if you are not sure), the BAM file must contain the XS tag on spliced reads. This is required for junction strand assignment in this workflow. To manually check if the XS tag is present in your BAM files, you can use ```samtools view [sample_name].bam | grep XS | head ```, which will display lines if XS is present. 
...
### Step 1: if you start from FASTQ files

Modify the job file submit_1_star_alignment_template.sh in the main work directory. For this step, you need the following: 

1. the absolute path of your input FASTQ directory. 

2. read length / maximum read length of paired end sequencing read mates (e.g. 150 for pair-end 150). 

3. -profile in the Nextflow command (at the bottom of the script) and corresponding config file. 

Now you are ready to go! 

```
qsub submit_1_star_alignment_[cohort name].sh   # or equivalent command for your scheduler
```

### Step 2: (Optional) Infer strandness from BAM files

If you are not sure about what strand parameters to use in the following splicing_junction_pipeline, run the helper script in [Strand-specificity](#strand-specificity)). 

### Step 3: If you start from aligned BAM files or you have generated BAM files as above

Modify the job file submit_3_splicing_junction_pipeline_template.sh in the main work directory. You need the following:  
...
```
qsub submit_3_splicing_junction_pipeline_[cohort name].sh   # or equivalent command for your scheduler
```
...
If you are not sure whether your RNA-seq library is stranded or not, please run the short diagnostic script named submit_2_strandness.sh. This script runs on BAM files, so the first step is to run the alignment pipeline if the starting point is FASTQ files. We use infer_experiment.py from [RSeQC](https://rseqc.sourceforge.net/). 

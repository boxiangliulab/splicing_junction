...
- Aligned BAM files (there is also an option to start from FASTQ, see below in [Configure](#configure))
...
2. strand parameter based on the results above. 


If you have aligned BAM files, modify the job file submit_splicing_junction_pipeline_[cohort name].sh in the main work directory (based on Option 1: Start from BAM files). 

1. the absolute path of your input BAM directory. 

2. -profile in the Nextflow command (at the bottom of the script) and corresponding config file. 
...
However, if you start from FASTQ files, modify the job file submit_splicing_junction_pipeline_[cohort name].sh in the main work directory (based on Option 2: Start from FASTQ files (two-step workflow)). In addition
